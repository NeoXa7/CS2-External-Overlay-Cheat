...
Vector2 AIMBOT::FindClosestEnemy()
{
    ...
    for (int i = 1; i < 64; i++)
    {
        ...
        Vector3 Head3D = aimbot.SetHeadCoordinates(BoneMatrix);
        Vector3 Neck3D = aimbot.SetNeckCoordinates(BoneMatrix);
        Vector3 Chest3D = aimbot.SetChestCoordinates(BoneMatrix);
        Vector3 Stomach3D = aimbot.SetStomachCoordinates(BoneMatrix);
        Vector3 Penis3D = aimbot.SetPenisCoordinates(entities->Position, currentPawn);
