...
namespace Config
{
	...
	{	
		...
		inline bool headBoneFilled = false;
		...
		ImColor HeadBoneColor = ImColor(255, 255, 255, 255); // Default white;
	...
	namespace Aimbot
	{
		...
		static int CurrentAimPosIndex; // Index of the currently selected item
		...
		Vector3 SetLShoulderCoordinates(uintptr_t boneMatrix)
		{
			Vector3 entityPosition = Memory::ReadMemory<Vector3>(boneMatrix + (BoneIndex::LSHOULDER * 0x20));
			return entityPosition;
		}

		Vector3 SetLArmCoordinates(uintptr_t boneMatrix)
		{
			Vector3 entityPosition = Memory::ReadMemory<Vector3>(boneMatrix + (BoneIndex::LARM * 0x20));
			return entityPosition;
		}

		Vector3 SetLHandCoordinates(uintptr_t boneMatrix)
		{
			Vector3 entityPosition = Memory::ReadMemory<Vector3>(boneMatrix + (BoneIndex::LHAND * 0x20));
			return entityPosition;
		}

		Vector3 SetRShoulderCoordinates(uintptr_t boneMatrix)
		{
			Vector3 entityPosition = Memory::ReadMemory<Vector3>(boneMatrix + (BoneIndex::RSHOULDER * 0x20));
			return entityPosition;
		}

		Vector3 SetRArmCoordinates(uintptr_t boneMatrix)
		{
			Vector3 entityPosition = Memory::ReadMemory<Vector3>(boneMatrix + (BoneIndex::RARM * 0x20));
			return entityPosition;
		}

		Vector3 SetRHandCoordinates(uintptr_t boneMatrix)
		{
			Vector3 entityPosition = Memory::ReadMemory<Vector3>(boneMatrix + (BoneIndex::RHAND * 0x20));
			return entityPosition;
		}

		Vector3 SetLThighCoordinates(uintptr_t boneMatrix)
		{
			Vector3 entityPosition = Memory::ReadMemory<Vector3>(boneMatrix + (BoneIndex::LTHIGH * 0x20));
			return entityPosition;
		}

		Vector3 SetLCalfCoordinates(uintptr_t boneMatrix)
		{
			Vector3 entityPosition = Memory::ReadMemory<Vector3>(boneMatrix + (BoneIndex::LCALF * 0x20));
			return entityPosition;
		}

		Vector3 SetLFootCoordinates(uintptr_t boneMatrix)
		{
			Vector3 entityPosition = Memory::ReadMemory<Vector3>(boneMatrix + (BoneIndex::LFOOT * 0x20));
			return entityPosition;
		}

		Vector3 SetRThighCoordinates(uintptr_t boneMatrix)
		{
			Vector3 entityPosition = Memory::ReadMemory<Vector3>(boneMatrix + (BoneIndex::RTHIGH * 0x20));
			return entityPosition;
		}

		Vector3 SetRCalfCoordinates(uintptr_t boneMatrix)
		{
			Vector3 entityPosition = Memory::ReadMemory<Vector3>(boneMatrix + (BoneIndex::RCALF * 0x20));
			return entityPosition;
		}

		Vector3 SetRFootCoordinates(uintptr_t boneMatrix)
		{
			Vector3 entityPosition = Memory::ReadMemory<Vector3>(boneMatrix + (BoneIndex::RFOOT * 0x20));
			return entityPosition;
		}
