...
{
	uintptr_t node = Memory::ReadMemory<uintptr_t>(entityPawn + Offsets::m_pGameSceneNode); // m_pGameSceneNode
	uintptr_t boneMatrix = Memory::ReadMemory<uintptr_t>(node + 0x1F0); // m_groundNormal 
	return boneMatrix;
}
