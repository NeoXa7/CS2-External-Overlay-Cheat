...
void Cheats::Risky_Cheats::Aimbot()
{
	...
	for (int i = 1; i < 64; i++)
	{
		...
		Vector3 entityPos = Memory::ReadMemory<Vector3>(currentPawn + Offsets::m_vOldOrigin);

		uintptr_t BoneMatrix = entities.BoneMatrix;

		Vector3 PenisPosition = Config::Aimbot::SetPenisCoordinates(entityPos, currentPawn);
