...
void CHEATS::SAFE_CHEATS::C4_Esp()
{
	...
	if (game.BombPlanted)
	{
			...
			WorldToScreen(PlantedC4HeightPos, PlantedC4Height, game.ViewMatrix))
		{
			...
			ESP::DrawBox(topLeftPBP, bottomRightPBP, Config::Esp::C4BoxColor, Config::Esp::C4BoxThickness);

			Vector2 namePos(Planted_C4_ScreenPos.x - (boxWidth / 2.0f), Planted_C4_ScreenPos.y);
			ESP::DrawTextW("C4_Planted", namePos, Config::Esp::C4BoxColor);
		}
	}
	...
	if (game.BombDropped)
	{
			...
			WorldToScreen(DroppedC4HeightPos, Dropped_C4_Height, game.ViewMatrix))
		{
			...
			ESP::DrawBox(topLeftDBP, bottomRightDBP, Config::Esp::C4BoxColor, Config::Esp::C4BoxThickness);

			Vector2 namePos(Dropped_C4_ScreenPos.x - (boxWidth / 2.0f), Dropped_C4_ScreenPos.y);
			ESP::DrawTextW("C4_Dropped", namePos, Config::Esp::C4BoxColor);
		}
	}
}
