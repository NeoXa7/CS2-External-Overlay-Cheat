        ...
        for (int i = 0; i < sizeof(boneConnections) / sizeof(boneConnections[0]); i++)
        {
                ...
                WorldToScreen(head, headScreenPos, game.ViewMatrix))
            {
                    ...
                    WorldToScreen(VectorBone2, b2, game.ViewMatrix))
                {
                    ...
                    {
                        if (game.DistanceFromLocalPlayerToEntities < Config::Esp::JointDisplayDistance)
                        {
                            ESP::DrawLine(b1, b2, Config::Esp::BonesColor, Config::Esp::BonesThickness, true, 2.0f, Config::Esp::BonesColor);
                        }
                        else
                        {
                            ESP::DrawLine(b1, b2, Config::Esp::BonesColor, Config::Esp::BonesThickness, false);
                        }
                    ...
                    {
                        ESP::DrawCircle(headScreenPos.x, headScreenPos.y, headHeight, Config::Esp::HeadColor, Config::Esp::BonesThickness);
                    }
                    else
                    {
                        ESP::DrawCircleFilled(headScreenPos.x, headScreenPos.y, headHeight, Config::Esp::HeadColor);
                    }
