...
private:
	...
	public:
		...
		{
			C_Elements::Checkbox("Enable C4 Esp", &Flags::enableC4Esp);
			if (Flags::enableC4Esp)
			{
				C_Elements::Checkbox("C4 Distance", &Config::Esp::enableC4Distance);
				C_Elements::Checkbox("C4 Status", &Config::Esp::enableC4StatusEsp);
			}
		}

		void GetEspSettingsElements()
		{
			C_Elements::BeginCustomizingCollapsingHeader();
			if (ImGui::CollapsingHeader("Esp Settings"))
			{
				...
				C_Elements::ColorEditor("Player Box Color", &Config::Esp::BoxColor);
				...
				C_Elements::ColorEditor("C4 Status Text Color", &Config::Esp::C4_Status_Text_Color);
				C_Elements::ColorEditor("C4 Distance Text Color", &Config::Esp::C4_Distance_Text_Color);
				C_Elements::LeaveLine(2);
		...
		void GetOtherVisualElements()
		{
			...
			{
				DrawCrosshair(Config::Game::CrosshairSize, Config::Game::CrosshairColor);
			}
		}
	};
	...
	public:
		...
		void GetGameInfo() // includes local player;
		{
			...
			ImGui::Text("Gamemode: %s", game.GetGameMode().c_str());
...
void RenderMenu()
{
	...
	if (Flags::showMenu)
	{
		ImGui::SetNextWindowSizeConstraints(ImVec2(450, 450), ImVec2(1366, 768));
		...
		if (ImGui::BeginTabBar("Cheats Type"))
		{
			...
			if (ImGui::BeginTabItem("Visuals"))
			{
				...
				visuals.GetEspSettingsElements();
				ImGui::EndTabItem();
			}
